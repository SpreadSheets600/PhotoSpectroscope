...
def absorbance_spectrun(entry, folder_name):
    # Plot Parameters
    ...
    for i in range(len(patchLabels)):
        patches.append(mpatches.Patch(
            color=plotColors[i], label=patchLabels[i]))
    ...
    plt.savefig(os.path.join(folder_name, entry.get() + ".png"))
    plt.show()


def transmittance_spectrun(entry, folder_name):
    # Plot Parameters
...
def reflectance_spectrun(entry, folder_name):
    # Plot Parameters
...
def generate_plot():
    ...
    generate_button.config(state="disabled", text="Enter The File Name Below")
...
# bg = PhotoImage(file=r"C:\Users\SOHAM\Desktop\Science Exhibition\Final Code\BG.png")

# root_label = ttk.Label(root, image=bg)
...
# Entry Parameters
name = ttk.Entry(root, state="disabled", justify="center", text="Enter The Name Of The File", style="Entry.TEntry")
name.place(x=150, y=130, width=300, height=30)

# File Saving Parameters
generate_button = ttk.Button(root, text="Generate Plot", command=lambda: [generate_plot()], style="Accen.TButton")
generate_button.place(x=150, y=90, width=300, height=30)
...
def GButton_364_command():
    ...
    absorbance_spectrun(name, folder_name)
...
def GButton_215_command():
    ...
    transmittance_spectrun(name, folder_name)
...
def GButton_597_command():
    ...
    reflectance_spectrun(name, folder_name)
...
GButton_364 = ttk.Button(root, state="disabled", style="Accent.TButton", text="Absorbance", command=lambda: [GButton_364_command()])
GButton_364.place(x=150, y=250, width=300, height=50)

# Defining The Transmittance Button

GButton_215 = ttk.Button(root, state="disabled", style="Accent.TButton", text="Transmittance", command=lambda: [GButton_215_command()])
GButton_215.place(x=150, y=310, width=300, height=50)

# Defining The Reflectance Button

GButton_597 = ttk.Button(root, state="disabled", style="Accent.TButton", text="Reflectance", command=lambda: [GButton_597_command()])
GButton_597.place(x=150, y=370, width=300, height=50)

# Label Parameters

ft = tkFont.Font(family="Consolas", size=30)
GLabel_525 = ttk.Label(root, style="Accent.TLabel", text="SPECTROSCOPY", justify="center", anchor="center", font=ft)
GLabel_525.place(x=150, y=30, width=300, height=50)
...
def next_steps():
    ...
    GMessage_530 = ttk.Button(root, state="disabled",style="Accent.TButton", text="Press R To Select ROI \nPress S After Selection \n\nPress Q To Quit", command=lambda: [root.destroy()])
    GMessage_530.place(x=150, y=450, width=300, height=100)
...
button = ttk.Button(root, text="Theme", command=change_theme)
